...
public class macroPass1 {
	public static void main(String[] Args) throws IOException{
		...
		HashMap<String,Integer> pntab=new HashMap<String,Integer>(); // parameter-name tab.
		String s;
			...
			if(word[0].compareToIgnoreCase("MACRO")==0){
				...
				for(int i=0;i<s.length();i++){
					if(s.charAt(i)=='&'){
						...
						i--;
						f3.write("#"+pntab.get(temp)); // writing to the mdt.txt file..
					}
					else
						f3.write(s.charAt(i));
				}
				...
				f1.write(s+'\n'); // writing to the intermediate.txt file
			}
		}
		b1.close();
		f1.close();
